document.addEventListener("DOMContentLoaded", () => {
  ...
  document
    .getElementById("formAddJogador")
    .addEventListener("submit", addJogador); // Adiciona o evento de submissão ao formulário de adicionar jogador
  document
    .getElementById("formAlterJogador")
    .addEventListener("submit", alterJogador); // Adiciona o evento de submissão ao formulário de adicionar jogador
  document
    .getElementById("reportType")
    .addEventListener("change", handleReportChange); // Adiciona o evento de mudança ao seletor de relatório
  document
    .getElementById("orderByEvents")
    .addEventListener("change", loadEventos); // Adiciona a filtragem a lista de eventos na busca
  document
    .getElementById("orderByAttack")
    .addEventListener("change", loadAtaques); // Adiciona a filtragem a lista de ataques na busca
  document.getElementById("orderByClans").addEventListener("change", loadClans); // Adiciona a filtragem a lista de clãs na busca
  document
    .getElementById("orderByPlayers")
    .addEventListener("change", loadJogadores); // Adiciona a filtragem a lista de jogadores na busca
});
    ...
    const actionButtons = actionColumn
      ...
      <td>
        <button class="update-btn" data-id="${
          item.ID_Jogador || item.ID_Cla || item.ID_Evento
...
function renderTableCla(tableId, headers, data, includeEdit = false) {
  const table = document.getElementById(tableId);
  table.innerHTML = headers;
  ...
  data.forEach((clan) => {
    const row = `
      <tr>
        <td><center><img src="${clan.Emblema}" alt="Emblema" width="70" height="70"></center></td>
        <td><center>${clan.ID_Cla}</center></td>
        <td>${clan.Nome}</td>
        <td>${clan.Nome_Liga}</td>
      </tr>
    `;
    table.innerHTML += row;
  });
...
async function loadJogadores() {
  ...
  const data = await fetchData(
    "http://localhost:3001/api/jogadores/" + orderByPlayers
  );
  const headers = `
    <tr>
      <th>ID</th>
...
async function loadClans() {
  ...
  const data = await fetchData(
    "http://localhost:3001/api/clans/" + orderByClans
  );
  const headers = `
    <tr>
      <th>Emblema</th>
  ...
  `;
  renderTableCla("clansTable", headers, data, false);
}
...
async function loadEventos() {
  ...
  const data = await fetchData(
    "http://localhost:3001/api/eventos/" + orderByEvents
  );
  const headers = `
    <tr>
      <th>Tipo</th>
...
async function loadAtaques() {
  ...
  const data = await fetchData(
    "http://localhost:3001/api/ataques/" + orderByAttack
  );
  const headers = `
    <tr>
        <th>ID</th>
...
async function deleteJogador(event) {
  ...
  try {
    ...
    const response = await fetch(`http://localhost:3001/api/jogadores/${id}`, {
      method: "DELETE",
    });
